document.addEventListener('DOMContentLoaded', function() {
    fetch('/api/dziekanat/zal6_lista')
        .then(response => {
    ...
    function renderTable(tbodyId, items, isActionable) {
        ...
        items.forEach(doc => {
            ...
            if (doc.status === 'Weryfikacja UOPZ' || doc.status === 'Weryfikacja Dziekanatu') {
            ...
            tr.innerHTML = `
                ...
                <td>${statusBadge}</td>
                <td class="text-end">
                    <a href="/dziekanat/dziennik/${doc.student_id}" class="btn btn-sm ${btnClass}">
