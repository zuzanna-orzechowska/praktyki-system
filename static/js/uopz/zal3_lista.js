document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTable(tbodyId, items, isActionable) {
        ...
        items.forEach(doc => {
            ...
            if (doc.status === 'Draft_UOPZ' || doc.status === 'Skierowanie_Wydane') {
                statusBadge = `<span class="badge bg-warning text-dark">Wydanie Skierowania</span>`;
            } else if (doc.status === 'Weryfikacja_ZOPZ') {
                statusBadge = `<span class="badge bg-info text-dark">W zakładzie pracy</span>`;
            } else if (doc.status === 'Weryfikacja_Uczelni') {
                statusBadge = `<span class="badge bg-primary">Ocena UOPZ</span>`;
            ...
            tr.innerHTML = `
                ...
                <td>${statusBadge}</td>
                <td class="text-end">
                    <a href="/uopz/zal3_karta/${doc.student_id}" class="btn btn-sm ${btnClass}">
